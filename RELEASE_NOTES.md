...
- Media type picker: **Meh** is highlighted only when it's **really** enabled.
- Extra fields Picker hidden in Gallery view.
- Pagination controls on desktop site are grouped into 3 subsections.
- Query timestamp and duration display: Show original **cached duration** and **completion time** for susequent queries which use the cached results.
- **Fixes:** Search / Duplicates / Uniques tabs session state:
  - Preserve pagination when switching between tabs.
  - Namespaced URL state; session holds inactive tabs while the URL reflects the active tab only.
- **Fix:** Page size dropdown includes **50** for duplicates and uniques.

### Profiles

- Dedicated `postgres` Spring profile removed.
- New prfile no-auth to disable login for develop[ment only

### Backend

#### Database & migrations (backend)
- **Flyway** migrations with a **single script set** for H2 and Postgres; datasource configuration via **environment**; Flyway on the base application configuration
